...
database.Guilds.findOne({ "_id": message.guild.id}, function(erro, sysop) {
    ...
    if (!sysop) {
        ...
        message.reply('<:SysopLogoEMOI:439565791357042700> Um histórico foi criado. Use o comando novamente!');
         ...
         // if (!amount) {
    if (!sysop.slow)
   return message.channel.send('<:sysalerta:469789950938841088> Este servidor não possui o slowmode ativado. Use `sy!slow help`');
   else 
   return message.channel.send(`O modo lento deste servidor está ativado num tempo de: ${sysop.slow} milisegundos. Use: sy!slow help para setar`);
