 ...
 if(message.author.bot) return;
    ...
    database.Users.findOne({
        ...
        if (documento) {
            ...
            if (message.mentions.users.size < 1) return message.reply("**Mencione alguem para doar rubys!**");
                ...
                if (args[1] < documento.rubys) {
                    ...
                    database.Users.findOne({
                        ...
                        if (docs) {
                            ...
                            docs.rubys += parseInt(args[1])
                            docs.save();
                            documento.rubys -= parseInt(args[1])
                            documento.save();
                            message.channel.send('<a:loading:442459946601152512> Transferindo...').then(function(value) {
                             setTimeout(function() {
                            value.edit(`<a:pblob:442467218282774548> | ${message.author} transferência concluída. Você pagou **${parseInt(args[1])}**  sycrystal <:sysopcrystal:482651147111366656> para <@${message.mentions.users.first().id}>`)}, 3000)
