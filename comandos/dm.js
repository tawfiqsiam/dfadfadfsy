...
database.Guilds.findOne({ "_id": message.guild.id}, function(erro, documento) {
   ...
   return message.channel.send('A mensagem de boas-vindas no privado atual é\n\`\`\`Markdown\n# ' + documento.welcome+'\`\`\`\n\nUse: sy!dm help para setar');
        ...
        if (mensagem !== 'disable') {
            ...
            documento.save();
            return message.channel.send(`GG ${message.author}! Você definiu a mensagem de boas-vindas no privado. <:LikeSysop3:462306728508260352>`);
        } else {
            documento.dm = '',
            documento.save();
            return message.channel.send('Mensagem de boas vindas no privado desabilitada.');
        }
