...
client.on('guildMemberAdd', member => {
	//contador + setador animado
database.Guilds.findOne({"_id": member.guild.id}, function(err0, sysop) {
	
    if(!sysop) return;
    if(!sysop.contadora) return;
    if (!sysop.texto)
...
let contadora = users.split('').map(i => ['<a:0:478771135388057602>', '<a:1:478771131030175746>', '<a:2:478771135714951169>', '<a:3:478771135865946130>', '<a:4:478771135454904335>', '<a:5:478771136247627806>', '<a:6:478771169441349632>', '<a:7:478771167382208524>', '<a:8:478771170263564288>', '<a:9:478771170557165578>'][i]).join('');
